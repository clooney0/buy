	...
	<div class="main">
		...
		<div class="manage">
			...
			<%
				...
				p.setPageSize(5);
				p.setPageIndex(pc);
				List<User> users = uDao.getAllUserLimitPage(p);
				for(User user : users){
			%>
				<tr>
					<td class="first w4 c"><%=user.getUserName() %></td>
					<td class="w1 c"><%=user.getName() %></td>
					<td class="w2 c"><%=user.getSex() %></td>
					<td><%=user.getEmail() %></td>
					<td class="w4 c"><%=user.getMobile() %></td>
					<td class="w1 c"><a href="user-modify.jsp?id=<%=user.getUserID()%>">修改</a> <a onclick='confirmDel()' href="deleteUser?id=<%=user.getUserID()%>">删除</a></td>
				</tr>
			<%} %>
			</table>
     ...
     <div class="pager">
				...
				<%
				     if(p.getPageIndex()>1){
				%>
				<li><a href="user.jsp?current=1">首页</a></li>
				<li><a href="user.jsp?current=<%=p.getPageIndex()-1 %>">上一页</a></li>
				<%} 
				if(p.getPageIndex()<p.getPageTotal()){ %>
				<li><a href="user.jsp?current=<%=p.getPageIndex()+1 %>">下一页</a></li>
				<li><a href="user.jsp?current=<%=p.getPageTotal() %>">尾页</a></li>
				<% }%>
				</ul>
			</div>
</div>
